...
namespace RedisHelper
{
    ...
    {
        private static readonly string RedisWriteConn = "RedisWriteConn";
        private static readonly string RedisReadConn = "RedisReadConn";
        private static readonly string RedisPrefix = "RedisPrefix";
        private static readonly object _lock = new object();

        private static IConfiguration Configuration { get; set; }

        public static RedisProvider Instance(IConfiguration configuration)
        {
        ...
        {
            return ConnectionMultiplexer.Connect(Configuration[RedisWriteConn].ToString());
        });
        ...
        {
            return ConnectionMultiplexer.Connect(Configuration[RedisReadConn].ToString());
        });

        /// <summary>
        /// 分布式session延时加载主
        /// </summary>
        private static Lazy<ConnectionMultiplexer> lazySessionWrite = null;

        /// <summary>
        /// 分布式session延时加载从
        /// </summary>
        private static Lazy<ConnectionMultiplexer> lazySessionRead = null;
        ...
        static RedisProvider()
        {
            try
            {
                ...
                Prefix = Configuration[RedisPrefix].ToString();
        ...
        public static void SetConfig(string master, string slave)
        {
            try
            {
                ...
                {
                    //sessionWriteRedis = ConnectionMultiplexer.Connect(Top.Core.Configs.AppSettings[master]);

                    lazySessionWrite = new Lazy<ConnectionMultiplexer>(() =>
                    {
                        return ConnectionMultiplexer.Connect(Configuration[master].ToString());
                    });
                }

                if (!string.IsNullOrEmpty(slave))
                {
                    //sessionReadRedis = ConnectionMultiplexer.Connect(Top.Core.Configs.AppSettings[slave]);

                    lazySessionRead = new Lazy<ConnectionMultiplexer>(() =>
                    {
                        return ConnectionMultiplexer.Connect(Configuration[slave].ToString());
                    });
                }
            }
            catch (Exception ex)
        ...
        public static void Clear(int dbId)
        {
            ...
            IServer server = writeRedis.GetServer(Configuration["MasterRedis"].ToString());
        ...
        /// <param name="dbId"></param>
        /// <returns></returns>
        public static bool StringSetSync(Dictionary<string, string> dict, int dbId = 1)
        {
            Dictionary<RedisKey, RedisValue> values = new Dictionary<RedisKey, RedisValue>();
        ...
        /// <param name="dbId"></param>
        /// <returns></returns>
        public static long StringIncrementSync(string key, long value, int dbId = 1)
        {
            key = SetPrefix(key);
        ...
        public static bool SessionKeyDelSync(string key, int dbId = 1)
        {
            key = SetPrefix(key);
        ...
        public static bool SessionKeyExistSync(string key, int dbId = 1)
        {
            key = SetPrefix(key);
        ...
        ///// <param name="dbId"></param>
        ///// <returns></returns>
        //public static bool HashSetSync(string key, Dictionary<string, object> dict, TimeSpan ts, int dbId = 1)
        //{
        ...
        //    foreach (var item in dict)
        //    {
        //        hashEntryArray[i] = new HashEntry(item.Key, Json.ToJsonString(item.Value, JsonBuilderSettings.SerializableType | JsonBuilderSettings.Default));
        //        i++;
        //    }
        ...
        public static bool HashSetSync(string key, string hashField, string value, int dbId = 1)
        {
            key = SetPrefix(key);
        ...
        public static string HashGetSync(string key, string hashField, int dbId = 1)
        {
            key = SetPrefix(key);
