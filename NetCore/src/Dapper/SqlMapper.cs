...
#pragma warning restore 618
        ...
        public static void ThrowDataException(Exception ex, int index, IDataReader reader, object value)
        {
            ...
            try
            {
                ...
                toThrow = new NullReferenceException(string.Format("Error parsing column {0} ({1}={2})", index, name, formattedValue), ex);
            }
            catch
            { // throw the **original** exception, wrapped as DataException
                toThrow = new NullReferenceException(ex.Message, ex);
            }
            throw toThrow;
        }
...
#pragma warning restore 612, 618
        ...
        public T Get<T>(string name)
        {
            ...
            if (val == DBNull.Value)
            {
                if (default(T) != null)
                {
                    throw new AggregateException("Attempting to cast a DBNull to a non nullable type! Note that out/return parameters will not have updated values until the data stream completes (after the 'foreach' for Query(..., buffered: false), or after the GridReader has been disposed for QueryMultiple)");
                }
                return default(T);
            }
            return (T)val;
