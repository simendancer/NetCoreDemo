...
namespace Tools.Utility
{
    public class CookieHelper
    {
        ...
        public static CookieHelper Instance(HttpContext context)
        {
            HttpContext = context;
        ...
        public static void SetObj(string strCookieName, int iExpires, string strValue, string strDomains, string strPath)
        {
            ...
            if (_strDomain.Length > 0)
                cookieOption.Domain = _strDomain;
            if (iExpires > 0)
            {
                if (iExpires == 1)
                    cookieOption.Expires = DateTime.MaxValue;
                else
                    cookieOption.Expires = DateTime.Now.AddSeconds(iExpires);
            }
            HttpContext.Response.Cookies.Append(strCookieName.Trim(), Uri.EscapeUriString(strValue.Trim()), cookieOption);
        }
        /// <summary>  
        /// 创建COOKIE对象并赋多个KEY键值  
        /// 设键/值如下：  
        ...
        public static void SetObj(string strCookieName, int iExpires, NameValueCollection KeyValue, string strDomains, string strPath)
        {
            ...
            if (_strDomain.Length > 0)
                cookieOption.Domain = _strDomain;
            cookieOption.Path = strPath.Trim();
            if (iExpires > 0)
            {
                if (iExpires == 1)
                    cookieOption.Expires = DateTime.MaxValue;
            ...
            {
                HttpContext.Response.Cookies.Append(strCookieName.Trim(), Uri.EscapeUriString(KeyValue[key].Trim()), cookieOption);
            }
        }
        /// <summary>  
        ...
        {
            if (HttpContext.Request.Cookies[strCookieName] == null)
            {
                return "";
            }
            else
            {
                string _value = HttpContext.Request.Cookies[strCookieName];
                return Uri.UnescapeDataString(_value);
            }
        }
        /// <summary>  
        ...
        public static string GetValue(string strCookieName, string strKeyName)
        {
            ...
            {
                string strObjValue = HttpContext.Request.Cookies[strCookieName];
        ...
        public static string Edit(string strCookieName, string strKeyName, string KeyValue, int iExpires, string strDomains, string strPath)
        {
            ...
            {
                CookieOptions cookieOption = new CookieOptions();
                cookieOption.Domain = strDomains;
                cookieOption.Path = strPath;
                if (iExpires > 0)
                {
                    if (iExpires == 1)
                        cookieOption.Expires = DateTime.MaxValue;
                ...
                HttpContext.Response.Cookies.Append(strCookieName.Trim(), Uri.EscapeUriString(KeyValue.Trim()), cookieOption);
                return "success";
            }
        }
        /// <summary>  
        ...
        public static void Del(string strCookieName, string strDomains, string strPath)
        {
            ...
            HttpContext.Response.Cookies.Append(strCookieName.Trim(), null, cookieOption);
        }
        /// <summary>  
        ...
        public static string DelKey(string strCookieName, string strKeyName, int iExpires)
        {
            ...
            {
                CookieOptions cookieOption = new CookieOptions();
                List<string> objCookie = HttpContext.Request.Cookies[strCookieName].Split(',').ToList();
                objCookie.RemoveAll(key => key == strKeyName);
                if (iExpires > 0)
                {
                    if (iExpires == 1)
                        cookieOption.Expires = DateTime.MaxValue;
                ...
                HttpContext.Response.Cookies.Append(strCookieName.Trim(), string.Join(",", objCookie), cookieOption);
                return "success";
            }
        }
        /// <summary>  
        ...
        private static string SelectDomain(string strDomains)
        {
            ...
            if (strDomains.Trim().Length == 0)
                return "";
            string _thisDomain = HttpContext.Request.Headers["SERVER_NAME"].ToString();
            //if (_thisDomain.IndexOf(".") < 0)//说明是计算机名，而不是域名  
            if (!_thisDomain.Contains("."))
                _isLocalServer = true;
            string _strDomain = "";//这个域名是瞎扯  
