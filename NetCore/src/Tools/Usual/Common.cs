...
namespace Tools.Usual
{
    public class Common
    {
        ...
        "U","V","W","X","Y","Z"
            ...
            //对传入网址进行MD5加密
            byte[] bs = System.Text.Encoding.UTF8.GetBytes(key + url);
            bs = MD5.Create().ComputeHash(bs);
            StringBuilder hex = new StringBuilder();
            foreach (byte b in bs)
            {
                hex.Append(b.ToString("x2").ToUpper());
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                int hexint = 0x3FFFFFFF & Convert.ToInt32("0x" + hex.ToString().Substring(i * 8, 8), 16);
        ...
        public static string CutString(string strInput, int intLen)
        {
            ...
            byte[] buffer1 = Encoding.UTF8.GetBytes(strInput);
            if (buffer1.Length > intLen)
            {
                string text1 = "";
                for (int num1 = 0; num1 < strInput.Length; num1++)
                {
                    byte[] buffer2 = Encoding.UTF8.GetBytes(text1 + strInput.Substring(num1, 1));
        ...
        public static async Task<float> DownLoadFile(string url, string savePath, string referer)
        {
            try
            {
                long ThisLength = 0;
                ...
                myHttpWebRequest.Headers["User-Agent"] = "Mozilla/4.0 (compatible; MSIE 6.01; Windows NT 5.0)";
                myHttpWebRequest.Headers["Referer"] = referer;
                myHttpWebRequest.ContinueTimeout = 10 * 1000;
                myHttpWebRequest.Method = "GET";


                HttpWebResponse res = await myHttpWebRequest.GetResponseAsync() as HttpWebResponse;
        ...
        {
            if (System.Text.Encoding.UTF8.GetByteCount(c.ToString()) == 2)
                return true;
            else
                return false;
        }
        ...
        public static byte GetTitleIndexInt(string title)
        {
            ...
            {
                if (Tools.Usual.StringCheck.IsNumber(titleIndex))
                {
